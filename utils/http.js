...
module.exports = {
  ...
  error(httpCode, res, err, opt_info = null) {
    ...
    if (Utils.isString(err)) {
      let {msgKey, message} = Utils.getMsgObjectFromKey(err, null, langCode);
      data = {msgKey, message};
    } else {
      if (Utils.isObject(err)) {
        ...
        if (msg) {
          data = {
            msgKey: msg,
            message: Utils.localizedText(msg, words, langCode)
          }
        } else {
          data = {msgKey, message}
        }
   ...
   */
  forwardResponse(res, {code = this.OK, message = "Success", data = null}) {
    if (code === this.OK) return this.sendResponse(res, code, {message, data})
  ...
  success(res, data = null, message = null, httpCode = this.OK) {
    let {langCode} = this.headers(res.req);
    message = message ? Utils.localizedText(message, null, langCode) : "Success";
    let ret = {message};
    if (data) {
      if (Utils.isString(data)) {
        ret.message = Utils.localizedText(data, null, langCode);
      ...
      if (Utils.isObject(data)) {
        if (Utils.isArray(data)) {
          ...
          if (Object.keys(data).length === 0) {
            ...
            if (msg) {
              ret.msgKey = msg;
              ret.message = Utils.localizedText(msg, words, langCode);
              delete data.msg;
              delete data.words;
            }
          }
...
function _error(res, httpCode, data) {
