...
class Message {
   ...
   * @method attachGrid
   *
   * @param  {Object}                 object : {content, filename, type, disposition}
   * @param  {Object}                 [options]
   *
   * @chainable
   *
   * @example
   * ```js
   * message.attachGrid({content: fs.createReadStream('hello.txt'), filename: 'hello.txt'})
   * ```
   */
  attachGrid(object, options) {
    if (!object.content) {
      throw GE
        .InvalidArgumentException
        .invalidParameter('Define content as 1st argument when calling message.attachGrid')
    }

    if (!object.filename) {
      throw GE
        .InvalidArgumentException
        .invalidParameter('Define filename as 2nd argument when calling message.attachGrid')
    }

    if (!object.type) object.type = "application/text"

    if (!object.disposition) object.disposition = "attachment"

    this.mailerMessage.attachments = this.mailerMessage.attachments || []

    const attachment = Object.assign(object, options || {})
    this.mailerMessage.attachments.push(attachment)
